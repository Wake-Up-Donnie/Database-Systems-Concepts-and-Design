...
<form id="myform" action="edit_shelter_sql.php" method="post">

<?php
$SiteID = $_SESSION['siteid'];
$conn = getConn();

$sql = "SELECT FacilityName, HoursOfOperation, MaleBunkAvailable, FemaleBunkAvailable, MixedBunkAvailable, FamilyRoomAvailable
FROM Shelter
...
if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
                ...
                echo "MaleBunkAvailable:  " . '<input type="text" name="MaleBunkAvailable" value="' . $row['MaleBunkAvailable'] . '"><br>';
                echo "FemaleBunkAvailable:  " . '<input type="text" name="FemaleBunkAvailable" value="' . $row['FemaleBunkAvailable'] . '"><br>';
		echo "MixedBunkAvailable:  " . '<input type="text" name="MixedBunkAvailable" value="' . $row['MixedBunkAvailable'] . '"><br>';
                echo "FamilyRoomAvailable:  " . '<input type="text" name="FamilyRoomAvailable" value="' . $row['FamilyRoomAvailable'] . '"><br>'; 
        }
}
...
<form action="delete_shelter.php" method = "post"><input type="submit" value="delete service" name="delete"></form>
